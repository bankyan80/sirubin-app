...
export async function GET(request: NextRequest) {
  try {
    // Auth check
    const authCheck = await getAuth(request)
    if (!authCheck.authenticated) return authCheck.response
    const adminCheck = requireAdmin(authCheck)
    ...
    const schoolFilters: Record<string, unknown> = { status: 'Aktif' }
    if (jenjang) schoolFilters.jenjang = jenjang
    if (kecamatan) schoolFilters.kecamatan = kecamatan

    const schools = await dbListAllSchools(schoolFilters)

    // 2. Get all laporan for this year matching filters
    const laporanFilters: Record<string, unknown> = { tahun: parseInt(tahun) }
    if (bulan) laporanFilters.bulan = parseInt(bulan)
    if (jenjang) laporanFilters.jenjang = jenjang

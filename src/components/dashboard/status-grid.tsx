...
export default function StatusGrid() {
  ...
  const handleCellClick = async (school: SchoolStatus, bulanIndex: number, monthData: MonthStatus) => {
    ...
    if (monthData.status === 'sudah' && monthData.laporanId) {
      ...
      try {
        ...
        const result = await res.json()
        if (result.success) {
          setLaporanDetail(result.data)
          setFeedbackInput(result.data.adminFeedback || '')
        }
      } catch {
        /* ignore */
      }
  ...
  // Save feedback (admin only)
  const handleSaveFeedback = async () => {
    if (!laporanDetail?.id) return
    setFeedbackSaving(true)
    try {
      const res = await apiFetch(`/api/laporan/${laporanDetail.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminFeedback: feedbackInput }),
      })
      const result = await res.json()
      if (result.success) {
        setLaporanDetail((prev) => prev ? { ...prev, adminFeedback: feedbackInput } : null)
  ...
  // Filter the data: ADMIN sees all schools, SEKOLAH sees all in their jenjang (handled by API), public sees all
              ...
              <div className="p-5">
                ...
                {selectedCell.monthData.status === 'sudah' && !detailLoading && laporanDetail && (
                  <div className="space-y-4">
                    ...
                    {isAdmin && (
                      <div>
                        <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mb-2">Feedback Admin</p>
                        <textarea
                          value={feedbackInput}
                          onChange={(e) => { setFeedbackInput(e.target.value); setFeedbackSaved(false) }}
                          placeholder="Tulis feedback untuk sekolah..."
                          rows={3}
                          className="w-full px-3 py-2.5 text-xs bg-slate-50/80 border border-slate-200/80 rounded-xl placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400/40 transition-all duration-200 resize-none"
                        />
                        <div className="flex items-center justify-between mt-2">
                          <span className={`text-[11px] font-medium transition-opacity ${feedbackSaved ? 'text-emerald-600' : 'text-transparent'}`}>
                            ✓ Feedback tersimpan
                          </span>
                          <button
                            onClick={handleSaveFeedback}
                            disabled={feedbackSaving || !feedbackInput.trim()}
                            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-white bg-gradient-to-r from-blue-600 to-violet-600 rounded-lg shadow-sm hover:shadow-md transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {feedbackSaving ? (
                              <>
                    ...
                    {/* School Feedback Display (for SEKOLAH role) */}
                    {!isAdmin && laporanDetail.adminFeedback && (
                      <div>
                        <p className="text-[11px] font-semibold text-blue-600 uppercase tracking-wider mb-2 flex items-center gap-1">
                          <FileText size={12} />
                          Feedback Admin
                        </p>
                        <p className="text-xs text-slate-700 bg-blue-50/60 border border-blue-100 rounded-lg p-2.5 whitespace-pre-wrap">{laporanDetail.adminFeedback}</p>
                      </div>
                    )}

...
import NikChecker from './NikChecker'
...
const tabs = [
  ...
  { id: 'cek-usia', label: 'Cek Usia', icon: Calendar },
  { id: 'cek-nik', label: 'Cek NIK', icon: FileSearch },
]
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex flex-wrap gap-1">
          {tabs.map((tab) => {
            ...
            // School can only see pendaftar, cek-usia, cek-nik tabs
            if (!isAdmin && !['pendaftar', 'cek-usia', 'cek-nik'].includes(tab.id)) return null
            return (
              <button
                key={tab.id}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.3 }}>
          ...
          {activeTab === 'cek-usia' && <CekUsiaTab />}
          {activeTab === 'cek-nik' && <CekNikTab />}
        </motion.div>
      </AnimatePresence>
    </div>
  )
